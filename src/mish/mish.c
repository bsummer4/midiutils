...
long find_marker(MidiFile_t midi_file, char *marker)
{
	...
	for (event = MidiFileTrack_getFirstEvent(MidiFile_getFirstTrack(midi_file)); event != NULL; event = MidiFileEvent_getPreviousEvent(event))
	{
	...
	while (1)
	{
		...
		else
		{
			...
			else if ((p = token), match_string(&p, "time=\"") && match_terminated_string(&p, '\"', strvalue1) && match_string(&p, "\"") && (*p == '\0'))
			{
				...
				if (marker_time < 0)
				{
					printf("Error at line %d of input file \"%s\" - no such marker.\n", Reader_getLineNumber(reader), Reader_getFilename(reader));
					exit(1);
				}
